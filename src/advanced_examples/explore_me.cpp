    ...
    if (inputStruct.c == "Attacker") {
        if (insecureEncrypt(inputStruct.a) == 0x4e9e91e6677cfff3L) {
            if (insecureEncrypt(inputStruct.b) == 0x4f8b9fb34431d9d3L) {
                // FIX: Remove or replace the call to trigger_double_free to prevent double-free vulnerability
                // trigger_double_free();
                // Alternatively, ensure proper memory management to avoid double-free issues.
            }
        }
    }
